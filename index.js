...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Yogiツ\n' //ganti nama lu! 
            + 'ORG: Owner XYTOD-Bot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=14706244520:+14706244520\n'  //ganti nomor lu! 
            + 'END:VCARD'
prefix = '#'
blocked = []   
limitawal = 99999
...
const LolhumanKey = '91b64af1c1afa0d0ad528536' //Kalau invalid beli sendiri / tunggu update
const TobzKey = 'QM3yCEiTr4LtX2fA70uV' //Kalau invalid beli sendiri / tunggu update

/******** OWNER NUMBER**********/
const ownerNumber = ["14706244520@s.whatsapp.net"]   //ganti nomor lu! 
const pacarNumber = ["14706244520@s.whatsapp.net"]  //ganti nomor lu!  
/************************************/
...
//dappa gay
	...
	break
		...
		case 'ttp':
				...
				ttp = await getBuffer(`http://lolhuman.herokuapp.com/api/ttp3?apikey=${LolhumanKey}&text=${body.slice(5)}`)
		...
		case 'spamcall':
			...
			break  
               ...
               if (!isRegistered) return reply( ind.noregis())
				...
				if (isBanned) return reply('Maaf kamu sudah terbenned!')
				if (!isPremium) return reply('*PREMIUM ONLY!*')
                    buffer = await getBuffer(`http://api.lolhuman.xyz/api/onecak?apikey=${LolhumanKey}`, {method: 'get'})
                    client.sendMessage(from, buffer, image, { quoted: mek })
                    break
               case 'ytmp3':
               if (!isRegistered) return reply( ind.noregis())
          ...
          if (!isRegistered) return reply( ind.noregis())
				...
				if (!isPremium) return reply('*PREMIUM ONLY!*')
					query = args.join(" ")
					anu = await fetchJson(`http://api.lolhuman.xyz/api/ytsearch?apikey=${LolhumanKey}&query=${query}`, {method: 'get'})
					teks = '=================\n'
...
2. Spam call,Spam command = *BLOCK + BANNED*` 
                    ...
                    client.sendMessage(from, wew, image, { quoted: freply, caption: yogi })
					break
					case 'soundmenu':
					if (!isRegistered) return reply( ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
...
case 'bneon':
		...
		case 'animehug':
				...
				if (args.length < 1) return reply(ind.wrongf())
                    ...
                    client.sendMessage(from, buffer, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_XYTOD BOT_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')} } }, caption: 'Nih hasilnya kak...'})
                    break
		...
		case 'hargapremium':
					...
					break
		case 'buypremium':
					client.sendMessage(from, 'Ingin membeli premium?\nHarap hubungi kami : wa.me/14706244520',MessageType.text, { quoted: mek} )
					break
		...
		case 'clone':
			...
			case 'pubglogo':
                ...
                ll2 = ct.split("/")[1];
				reply(ind.wait())
				anu = await fetchJson(`https://yogipw-api.herokuapp.com/api/textmaker/game?text=${ll1}&text2=${ll2}&theme=pubg&apikey=${YogiPwKey}`)
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				ll2 = ct.split("/")[1];
				reply(`[❕] Loading`)
				anu = await fetchJson(`https://yogipw-api.herokuapp.com/api/textmaker/game?text=${ll1}&text2=${ll2}&theme=battlefield&apikey=${YogiPwKey}`)
				buffer = await getBuffer(anu.result.url)
				client.sendMessage(from, buffer, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "_XYTOD BOT_", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('image/odc.jpeg')} } }, caption: 'Nih hasilnya kak...'})
				break
				default:
